...
class SubPluginTestCase(TestCase):
    def test_model_inheritance(self):
        self.assertTupleEqual(
            tuple1=SubPlugin.__bases__,
            tuple2=(Project,),
        )

    def test_field_names(self):
        self.assertSetEqual(
            set1=get_new_fields_for_model(SubPlugin),
            set2={
                "id",
                "basename",
                "owner",
    ...
    def test_id_field(self):
        field = SubPlugin._meta.get_field("id")
        self.assertEqual(
            first=field.max_length,
            second=PROJECT_SLUG_MAX_LENGTH * 2 + 1,
        )
        self.assertTrue(
            expr=field.blank,
        )
        self.assertTrue(
            expr=field.primary_key,
        )
    ...
    def test_meta_class(self):
        ...
        self.assertTupleEqual(
            tuple1=getattr(SubPlugin._meta, "unique_together"),
            tuple2=(
                ("plugin", "basename"),
                ("plugin", "name"),
                ("plugin", "slug"),
